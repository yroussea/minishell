...
static size_t	ft_format_fd(const char c, va_list args, int fd)
{
	...
	if (c == 's')
		return (ft_putlst_fd(va_arg(args, char *), fd));
	if (c == 'u')
		return (ft_putlunbr_fd(va_arg(args, unsigned int), fd));
	if (c == 'd' || c == 'i')
		return (ft_putlnbr_fd(va_arg(args, int), fd));
	if (c == '%')
		return (ft_putlchar_fd('%'));
	if (c == 'x')
		return (ft_putlhexa_low(va_arg(args, unsigned int), fd));
	if (c == 'X')
	...
	if (!c)
		return (-1);
	return (ft_putlchar_fd('%' + ft_putlchar(c)));
}
